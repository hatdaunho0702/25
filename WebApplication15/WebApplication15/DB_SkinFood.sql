...
-- ==================================================================================
-- BƯỚC 1: LÀM SẠCH (Reset lại DB_SkinFood1)
-- ==================================================================================
...
-- ==================================================================================
-- BƯỚC 2: TẠO BẢNG (GIỮ NGUYÊN TÊN CŨ CỦA BẠN)
-- ==================================================================================
...
CREATE TABLE NguoiDung (
    ...
    SoDienThoai NVARCHAR(20), -- Đã có cột này
    DiaChi NVARCHAR(255),
    GioiTinh NVARCHAR(10),
    NgaySinh DATE,            -- Đã có cột này
...
-- 7. SẢN PHẨM (Đầy đủ cột để không lỗi Code)
CREATE TABLE SanPham (
    ...
    MaDM INT, MaTH INT, MaLoai INT,
    -- Chi tiết mỹ phẩm
    NgaySanXuat DATETIME, HanSuDung DATETIME,
    LoaiDa NVARCHAR(50), 
...
CREATE TABLE DonHang (
    MaDH INT PRIMARY KEY IDENTITY(1,1), MaND INT, NgayDat DATETIME DEFAULT GETDATE(), TongTien DECIMAL(18,2) DEFAULT 0,
    DiaChiGiaoHang NVARCHAR(255), TenNguoiNhan NVARCHAR(100), SoDienThoai NVARCHAR(20), GhiChu NVARCHAR(255),
    TrangThaiThanhToan NVARCHAR(50) DEFAULT N'Chưa thanh toán', NgayThanhToan DATETIME NULL, PhuongThucThanhToan NVARCHAR(100) NULL,
    FOREIGN KEY (MaND) REFERENCES NguoiDung(MaND)
);
GO

-- 9. CHI TIẾT ĐƠN HÀNG (Giữ tên có chữ 's' ở cuối: ChiTietDonHangs)
CREATE TABLE ChiTietDonHangs (
    MaDH INT, MaSP INT, SoLuong INT CHECK (SoLuong > 0), DonGia DECIMAL(18,2),
    PRIMARY KEY (MaDH, MaSP), FOREIGN KEY (MaDH) REFERENCES DonHang(MaDH) ON DELETE CASCADE, FOREIGN KEY (MaSP) REFERENCES SanPham(MaSP)
);
GO

-- 10. PHIẾU NHẬP
CREATE TABLE PhieuNhap (
    MaPN INT PRIMARY KEY IDENTITY(1,1), MaND INT, MaNCC INT, NgayNhap DATETIME DEFAULT GETDATE(), TongTien DECIMAL(18,2) DEFAULT 0, GhiChu NVARCHAR(500),
    FOREIGN KEY (MaND) REFERENCES NguoiDung(MaND), FOREIGN KEY (MaNCC) REFERENCES NhaCungCap(MaNCC)
);
GO

-- 11. CHI TIẾT PHIẾU NHẬP
CREATE TABLE ChiTietPhieuNhap (
    MaPN INT, MaSP INT, SoLuong INT CHECK (SoLuong > 0), GiaNhap DECIMAL(18,2) CHECK (GiaNhap >= 0), ThanhTien DECIMAL(18,2),
    PRIMARY KEY (MaPN, MaSP), FOREIGN KEY (MaPN) REFERENCES PhieuNhap(MaPN) ON DELETE CASCADE, FOREIGN KEY (MaSP) REFERENCES SanPham(MaSP)
);
GO

-- 12. PHIẾU XUẤT & CHI TIẾT
CREATE TABLE PhieuXuat (MaPX INT PRIMARY KEY IDENTITY(1,1), MaND INT, NgayXuat DATETIME DEFAULT GETDATE(), LyDoXuat NVARCHAR(255), GhiChu NVARCHAR(500), FOREIGN KEY (MaND) REFERENCES NguoiDung(MaND));
CREATE TABLE ChiTietPhieuXuat (MaPX INT, MaSP INT, SoLuong INT CHECK (SoLuong > 0), PRIMARY KEY (MaPX, MaSP), FOREIGN KEY (MaPX) REFERENCES PhieuXuat(MaPX) ON DELETE CASCADE, FOREIGN KEY (MaSP) REFERENCES SanPham(MaSP));
GO

-- 13. BẢNG PHỤ
CREATE TABLE DanhGia (MaDG INT PRIMARY KEY IDENTITY(1,1), MaSP INT, MaND INT, NoiDung NVARCHAR(500), Diem INT, NgayDanhGia DATETIME DEFAULT GETDATE(), DuocApprove BIT DEFAULT 0, TraLoiAdmin NVARCHAR(500), ThoiGianTraLoi DATETIME, FOREIGN KEY (MaSP) REFERENCES SanPham(MaSP), FOREIGN KEY (MaND) REFERENCES NguoiDung(MaND));
...
-- ==================================================================================
-- BƯỚC 3: TRIGGER (LOGIC CHUẨN - KHÔNG SỬA TÊN)
-- ==================================================================================
...
-- Trigger Bán Hàng (Áp dụng cho bảng ChiTietDonHangs)
...
-- ==================================================================================
-- BƯỚC 4: DỮ LIỆU MẪU (KHỚP VỚI ẢNH VÀ ADMIN)
-- ==================================================================================
...
(1, 1, 100, 200000, 20000000), (1, 2, 50, 800000, 40000000), (1, 3, 100, 100000, 10000000),
(1, 4, 100, 120000, 12000000), (1, 5, 100, 150000, 15000000), (1, 6, 100, 100000, 10000000),
(1, 7, 50, 600000, 30000000), (1, 8, 100, 300000, 30000000), (1, 9, 50, 500000, 25000000), (1, 10, 100, 250000, 25000000);

PRINT N'✅ Đã khôi phục về DB_SkinFood1 thành công!';