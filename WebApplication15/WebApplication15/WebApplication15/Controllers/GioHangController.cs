...
namespace WebApplication15.Controllers
{
    public class GioHangController : Controller
    {
        ...
        public ActionResult PaymentConfirm(string tenNguoiNhan, string soDienThoai, string diaChi, string ghiChu, string phuongThuc = "COD")
        {
            ...
            {
                var spCheck = data.SanPhams.Find(item.MaSP);
                int available = spCheck?.SoLuongTon ?? 0;
                if (available < item.SoLuong)
                {
                    TempData["Error"] = $"Lỗi: Sản phẩm '{item.TenSP}' chỉ còn {available} nhưng bạn yêu cầu {item.SoLuong}. Vui lòng điều chỉnh giỏ hàng.";
                    // update session cart quantities to reflect reality
                    try
                    {
                        var cartCurrent = Session["Cart"] as Cart;
                        if (cartCurrent != null)
                        {
                            var it = cartCurrent.list.FirstOrDefault(x => x.MaSP == item.MaSP);
                            if (it != null)
                            {
                                if (available <= 0) cartCurrent.list.Remove(it);
                                else it.SoLuong = available;
