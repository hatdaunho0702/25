...
namespace WebApplication15.Services
{
    public class CouponService
    {
        ...
        public decimal CalculateDiscount(string couponCode, List<CartItem> cartItems, decimal totalOrderValue)
        {
            ...
            // Fallback: tolerant lookup (trim + case-insensitive). Load into memory then compare to avoid EF translation issues.
            if (km == null)
            {
                var trimmed = couponCode?.Trim();
                km = _db.KhuyenMais.ToList().FirstOrDefault(k => string.Equals(k.MaCode?.Trim(), trimmed, StringComparison.OrdinalIgnoreCase));
            }
